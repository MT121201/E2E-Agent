...
def validate_config_file(file: FilePath) -> FilePath:
...
class Config(BaseModel):
    ...
    """
    model_config = ConfigDict(arbitrary_types_allowed=True, validate_assignment=True, validate_default=True, extra="forbid")
    ...
    @classmethod
    @handle_exception(ConfigException, message="Invalid Configuation")
    def init_config(clas, data: dict)-> dict:
        config = {}
        if data.get('file') is not None:
            file = validate_config_file(data['file'])
